...
SELECT
    ...
    coalesce(any(st.latest_stock), 0) AS latest_stock,
    multiIf(
        coalesce(avg(if(s.day >= today() - 14, s.qty, NULL)), 0) > 0,
        coalesce(any(st.latest_stock), 0) / avg(if(s.day >= today() - 14, s.qty, NULL)),
        999
    ) AS days_until_stockout,
    multiIf(
        coalesce(avg(if(s.day >= today() - 14, s.qty, NULL)), 0) > 0
        AND coalesce(any(st.latest_stock), 0) < avg(if(s.day >= today() - 14, s.qty, NULL)) * 21,
        toUInt32(ceil(avg(if(s.day >= today() - 14, s.qty, NULL)) * 30 - coalesce(any(st.latest_stock), 0))),
        0
    ) AS reorder_qty,
    multiIf(
        coalesce(any(st.latest_stock), 0) <= 0, 1,
        coalesce(avg(if(s.day >= today() - 14, s.qty, NULL)), 0) > 0
        AND coalesce(any(st.latest_stock), 0) / avg(if(s.day >= today() - 14, s.qty, NULL)) < 14, 1,
        0
    ) AS is_critical
