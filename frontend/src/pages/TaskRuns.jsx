...
export default function TaskRuns() {
  ...
  if (!rows.length) return <EmptyState message={t("taskRuns.empty")} />;
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <MetricCard label={t("taskRuns.totalRuns")} value={numberFmt(rows.length)} />
        <MetricCard label={t("taskRuns.success")} value={numberFmt(success)} accent="success" />
        <MetricCard label={t("taskRuns.failed")} value={numberFmt(failed)} accent="error" />
        <MetricCard label={t("taskRuns.successRate")} value={rows.length ? percentFmt(success / rows.length) : "0%"} accent={success >= failed ? "success" : "error"} />
      </div>
      <section className="md3-card-elevated p-4">
        <h3 className="text-sm font-semibold text-[var(--color-on-surface)] mb-3">{t("taskRuns.title")}</h3>
