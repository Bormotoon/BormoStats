  ...
  return (
    <div className="overflow-auto rounded-xl border border-[var(--color-border)]">
      <table className="w-full min-w-[640px] text-sm">
        <thead>
          <tr className="bg-[var(--color-surface-highest)]">
            {columns.map((col) => (
              <th
                key={col}
                className="text-left px-3 py-2.5 text-xs font-bold text-[var(--color-text-muted)] uppercase tracking-wider sticky top-0 bg-[var(--color-surface-highest)] z-10"
              >
                {col}
              </th>
            ))}
        ...
        <tbody className="divide-y divide-[var(--color-border)]">
          {rows.map((row, i) => (
            <tr
              key={i}
              className="hover:bg-[var(--color-surface-high)]/50 transition-colors"
            >
              {columns.map((col) => (
